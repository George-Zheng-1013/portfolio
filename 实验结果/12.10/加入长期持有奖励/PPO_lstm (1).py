...
def preprocess_data(df_, tech_cols):
    ...
    features = pd.concat(
        [returns, volatility, rsi_df, macd_df, bias_df, quantile_df, corr_df, alpha_df, macro_df],
        axis=1,
    )
...
def objective(trial: optuna.Trial) -> float:
    ...
    dip_bonus_coef = trial.suggest_float("dip_bonus_coef", 0.0, 0.1) # 抄底奖励系数
    downside_risk_coef = trial.suggest_float("downside_risk_coef", 0.0, 20.0) # 下行风险惩罚系数
...
def main():
    ...
    if len(balances_arr) > 0:
        ...
        if len(weights_history) > 0:
            ...
            plt.stackplot(df_weights.index, df_weights.T, labels=df_weights.columns, edgecolor="none")
            plt.title(
