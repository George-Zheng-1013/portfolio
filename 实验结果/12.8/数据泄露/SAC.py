...
class PortfolioOptimizationEnv(gym.Env):
    """
    组合优化环境：
    - 支持 High-Alpha 奖励（动量 / 趋势）
    - 支持 Top-K 强势股 Winner-Take-All
    - 支持 Optuna 调节 reward / k_top
    """
    ...
    def __init__(
        ...
        reward_scale=50.0,
        # ---- high-alpha 参数 ----
        k_top=2,      # 只重仓动量最高的 K 个资产，K 将由 Optuna 优化
        seed=None,
    ):
        super().__init__()
    ...
    def step(self, raw_action):
        ...
        # ================================
        # High-Alpha：计算动量评分（强势股得分）
        # ================================
        feat_row = self.feature_data.iloc[self.current_step]

        mom5 = feat_row[[col + "_mom5" for col in TECH_COLS]].values
        mom20 = feat_row[[col + "_mom20" for col in TECH_COLS]].values
        mom60 = feat_row[[col + "_mom60" for col in TECH_COLS]].values

        momentum_score_tech = 0.2 * mom5 + 0.3 * mom20 + 0.5 * mom60
        momentum_score_tech = np.nan_to_num(momentum_score_tech, 0.0)

        # 把动量分数映射到完整 tickers（US_debt / Cash 的分数为 0）
        ...
        # ================================
        # Top-K Winner-Take-All（只重仓最强的 K 个）
        # ================================
        if (
            self.k_top is not None
            and 1 <= self.k_top < len(self.tickers)
        ):
            k = self.k_top
            top_idx = np.argsort(momentum_score_full)[-k:]
...
# Train / test split
# =========================================
tickers = df.columns.tolist()  # 包含所有资产：股+债+指数+黄金+现金
window_size = 30
initial_balance = 10000.0
seed = 8
...
def objective(trial: optuna.Trial) -> float:
    ...
    # ---- High-Alpha: Top-K 强势股数目 ----
    k_top = trial.suggest_int("k_top", 1, 3)
...
def main():
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="sac_portfolio_tuning_high_alpha",
    )
    ...
    def make_best_env():
        def _init():
            env_ = PortfolioOptimizationEnv(
                ...
                k_top=best_params["k_top"],
                seed=seed,
            )
            return env_
        return _init

    log_dir = "./sb3_logs_optuna_high_alpha"
    ...
    if len(balances_arr) > 0:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("backtest_high_alpha.png")
        plt.show()
        ...
        if len(weights_history) > 0:
            ...
            plt.title(
                "Asset Allocation Evolution (Position Weights) - High Alpha",
                fontsize=16,
                pad=20,
            )
            plt.xlabel("Date")
            ...
            plt.tight_layout()
            plt.savefig("weights_high_alpha.png")
            plt.show()
