...
class PortfolioOptimizationEnv(gym.Env):
    ...
    def step(self, raw_action):
        ...
        # softmax
        temp = self.lambda_softmax
        exp_a = np.exp((raw_action - np.max(raw_action)) / temp)
        ...
        if len(self.return_window) > 5:
            asset_vol = np.std(np.array(self.return_window), axis=0)
        else:
            asset_vol = np.zeros(len(self.tickers))

        variance = port_ret**2
        variance = float(
            np.nan_to_num(variance, nan=0.0, posinf=0.0, neginf=0.0)
        )
        asset_vol = np.nan_to_num(
            asset_vol, nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        if self.max_balance == 0:
            ...
            drawdown = float(
                (self.balance - self.max_balance) / self.max_balance
            )
        drawdown = float(
            np.nan_to_num(drawdown, nan=0.0, posinf=0.0, neginf=0.0)
        )

        # risk budgeting
        risk_contrib = action * asset_vol
        risk_contrib = np.nan_to_num(
            risk_contrib, nan=0.0, posinf=0.0, neginf=0.0
        )
        if np.all(risk_contrib == 0):
            rb_penalty = 0.0
        else:
            target_risk = np.mean(risk_contrib)
            rb_penalty = -float(
                np.mean(np.abs(risk_contrib - target_risk))
            )
        rb_penalty = float(
            np.nan_to_num(rb_penalty, nan=0.0, posinf=0.0, neginf=0.0)
        )

        # reward with tunable weights
        reward = (
            self.w_ret * port_ret
            + self.w_down * downside
            + self.w_dd * drawdown
            + self.w_rb * rb_penalty
            - self.w_var * variance
...
def objective(trial: optuna.Trial) -> float:
    ...
    w_ret = trial.suggest_float("w_ret", 0.5, 2.0)
    w_down = trial.suggest_float("w_down", 0.0, 1.0)
    w_dd = trial.suggest_float("w_dd", 0.0, 1.0)
    w_rb = trial.suggest_float("w_rb", 0.0, 1.0)
    w_var = trial.suggest_float("w_var", 0.0, 50.0)
    w_turn = trial.suggest_float("w_turn", 0.0, 0.5)
    reward_scale = trial.suggest_float("reward_scale", 10.0, 80.0)

    lambda_softmax = trial.suggest_float("lambda_softmax", 0.5, 5.0)
    ...
    def make_best_env():
        def _init():
            env_ = PortfolioOptimizationEnv(
                ...
                feature_df_all=full_features,
                initial_balance=initial_balance,
                lambda_softmax=best_params["lambda_softmax"],
                w_ret=best_params["w_ret"],
                w_down=best_params["w_down"],
                w_dd=best_params["w_dd"],
                w_rb=best_params["w_rb"],
